...
describe('highlightText', function () {
  ...
  describe('extractText()', function () {
    ...
    it('extracts the text', function () {
      element.innerHTML = 'a'
      const text = highlightText.extractText(element)
      expect(text).toBe('a')
    })

    it('extracts the text with nested elements', function () {
      element.innerHTML = 'a<span>b</span><span></span>c'
      const text = highlightText.extractText(element)
    ...
    it('extracts a &nbsp; entity', function () {
      element.innerHTML = '&nbsp;'
      const text = highlightText.extractText(element)
      expect(text).toBe('\u00A0') // \u00A0 is utf8 for the '&nbsp;' html entity
    })
    ...
    it('skips stored cursor positions', function () {
      element = createElement('<div>ab</div>')
      const cursor = createCursor(element, element.firstChild, 1)
      cursor.save()
      const text = highlightText.extractText(element)
      expect(text).toBe('ab')
    })
    ...
    it('extracts text with a <br> properly', function () {
      element = createElement('<div>a<br>b</div>')
      const text = highlightText.extractText(element)
      expect(text).toBe('a\nb')
    })

    it(`extracts ' <br>' properly`, function () {
      element = createElement('<div> <br></div>')
      const text = highlightText.extractText(element)
      expect(text).toBe(' \n')
    })
  ...
  describe('iterator', function () {
    ...
    beforeEach(function () {
      wrapMatch = vi.spyOn(highlightText, 'wrapMatch')
    })
  ...
  describe('wrapMatch()', function () {
    it('wraps a word in a single text node', function () {
      ...
      removeWordId(elem)
      expect(elem.outerHTML).toBe('<div>Some <span spellcheck="true">juice</span>.</div>')
    })
    ...
    it('wraps a word with a partial <em> element', function () {
      ...
      expect(elem.outerHTML).toBe(
        '<div>Some <span spellcheck="true">jui</span><em><span spellcheck="true">ce</span>.</em></div>'
      )
    })
    ...
    it('wraps two words in the same text node', function () {
      ...
      removeWordId(elem)
      expect(elem.outerHTML).toBe(
        '<div><span spellcheck="true">a</span> or <span spellcheck="true">b</span></div>'
      )
    })
    ...
    it('wraps a word in a <em> element', function () {
      ...
      highlight(elem, ['word'])
      removeWordId(elem)
      expect(elem.outerHTML).toBe('<div><em><span spellcheck="true">word</span></em></div>')
    })
    ...
    it('can handle a non-match', function () {
      ...
      highlight(elem, ['xxx'])
      removeWordId(elem)
      expect(elem.outerHTML).toBe('<div><em>word</em></div>')
    })
    ...
    it('works with a more complex regex', function () {
      ...
      removeWordId(elem)
      expect(elem.outerHTML).toBe(
        '<div><em><span spellcheck="true">a</span></em> or <span spellcheck="true">b</span></div>'
      )
    })
    ...
    it('wraps two words with a tag in between', function () {
      ...
      removeWordId(elem)
      expect(elem.outerHTML).toBe(
        '<div>A <span spellcheck="true">word</span> <em>is</em> <span spellcheck="true">not</span> necessary</div>'
      )
    })
    ...
    it('wraps two characters in the same textnode, when the first match has an offset', function () {
      ...
      removeWordId(elem)
      expect(elem.outerHTML).toBe(
        '<div>a, <span spellcheck="true">b</span> or <span spellcheck="true">c</span>, d</div>'
      )
    })
    ...
    it('wraps a character after a <br>', function () {
      ...
      highlight(elem, ['b'])
      removeWordId(elem)
      expect(elem.outerHTML).toBe('<div>a<br><span spellcheck="true">b</span></div>')
    })
    ...
    it('stores data-word-id on a highlight', function () {
      ...
      expect(elem.outerHTML).toBe('<div><span data-word-id="0">a</span></div>')
    })
    ...
    it('stores data-word-id on different matches', function () {
      ...
      removeSpellcheckAttr(elem)
      expect(elem.outerHTML).toBe(
        '<div><span data-word-id="0">a</span> <span data-word-id="2">b</span></div>'
      )
    })
    ...
    it('stores same data-word-id on multiple highlights for the same match', function () {
      ...
      removeSpellcheckAttr(elem)
      expect(elem.outerHTML).toBe(
        '<div><span data-word-id="0">a</span><i><span data-word-id="0">b</span></i></div>'
      )
    })
  })
})
